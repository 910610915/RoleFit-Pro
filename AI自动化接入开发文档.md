...
1.  **用户体验 (UX)**
    -   **自定义看板**: 引入 `Widget Board`，支持管理员和经理基于角色配置不同的监控卡片。
    -   **交互式图表**: 使用 ECharts 提供可缩放、可筛选的时间序列数据展示。

2.  **测试流程控制**
    -   **前置检查 (Pre-flight Checks)**: 强制检查系统闲置状态（如鼠标键盘无操作 5 分钟），避免干扰员工工作。
    -   **资源节流 (Resource Throttling)**: 在下发测试任务时，允许指定 CPU/内存的使用上限（如 80%），防止测试过程导致系统卡死。
