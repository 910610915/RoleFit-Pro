...
# 数据库配置
DATABASE_URL=postgresql+asyncpg://benchmark_user:benchmark_pass_2026@postgres:5432/hardware_benchmark
DATABASE_URL_SYNC=postgresql://benchmark_user:benchmark_pass_2026@postgres:5432/hardware_benchmark
...
INFLUXDB_ORG=hardware-benchmark
...
# PostgreSQL连接检查
docker-compose exec postgres pg_isready -U benchmark_user
...
docker-compose exec postgres pg_dump -U benchmark_user hardware_benchmark > \
...
```bash
# 恢复数据
docker-compose exec -T postgres psql -U benchmark_user hardware_benchmark < backup_20260223.sql
```
...
```bash
# 导出数据
docker-compose exec postgres pg_dump -U benchmark_user hardware_benchmark > dump.sql

# 导入到新服务器
docker-compose exec -T postgres psql -U benchmark_user hardware_benchmark < dump.sql
```
...
# 4. 检查端口监听
netstat -tlnp | grep -E '(8000|5432|6379|80)'
...
# 删除数据卷（⚠️会删除所有数据）
docker volume rm hardware-benchmark_postgres_data
docker volume rm hardware-benchmark_influxdb_data
