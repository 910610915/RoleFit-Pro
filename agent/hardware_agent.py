...
try:
    from nodejs_hardware import get_hardware_info as get_hardware_info_nodejs

    NODEJS_AVAILABLE = True
except ImportError:
    NODEJS_AVAILABLE = False
    get_hardware_info_nodejs = None
...
class HardwareCollector:
    ...
    def get_cpu_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get CPU base clock from WMI
            cpu_base_clock = None
            try:
                # Try to get from WMI
                if hasattr(cpu, "MaxClockSpeed") and cpu.MaxClockSpeed:
                    cpu_base_clock = float(cpu.MaxClockSpeed)  # MHz
            except:
                pass

            # Fallback: try psutil
            if not cpu_base_clock:
                try:
                    freq = psutil.cpu_freq()
                    if freq:
                        cpu_base_clock = freq.max  # Max frequency in MHz
                except:
                    pass
            ...
            return {
                ...
                "cpu_base_clock": cpu_base_clock,  # MHz
            }
    ...
    def get_memory_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for mem in memory:
                ...
                # Get memory speed (frequency)
                if hasattr(mem, "Speed") and mem.Speed:
                    mem_info["speed"] = mem.Speed  # MHz
                    if ram_frequency is None:
                        ram_frequency = mem.Speed
                ...
                if hasattr(mem, "SMBIOSMemoryType") and mem.SMBIOSMemoryType:
                    ...
                    mem_info["type"] = mem_type_map.get(
                        mem.SMBIOSMemoryType, f"Type_{mem.SMBIOSMemoryType}"
                    )

                # Get manufacturer
                if hasattr(mem, "Manufacturer") and mem.Manufacturer:
                    mem_info["manufacturer"] = mem.Manufacturer.strip()

                memory_list.append(mem_info)

            return {
                "ram_total_gb": total_gb,
                "ram_frequency": ram_frequency,  # MHz
                "ram_sticks": ram_sticks,
                "all_memory": memory_list,
            }
        except Exception as e:
        ...
        try:
            ...
            if primary_gpu:
                return {
                    ...
                    "gpu_vram_mb": int(
                        primary_gpu["vram_gb"] * 1024
                    ),  # Convert back to MB for compatibility
                    "gpu_driver_version": primary_gpu.get("driver_version"),
                    "all_gpus": gpu_list,
                }
            return {}
        except Exception as e:
    ...
    def get_disk_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for disk in disks:
                ...
                interface = (
                    disk.Description
                )  # hardware_check uses Description as interface
                model = disk.Model
    ...
    def get_os_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                os_version = (
                    f"{display_version} {edition}".strip()
                    if display_version or edition
                    else uname.release
                )
    ...
    def get_all_hardware_info(self) -> Dict[str, Any]:
        """Get all hardware information - prefer Node.js systeminformation"""
        # Try Node.js systeminformation first (more accurate)
        if NODEJS_AVAILABLE and get_hardware_info_nodejs:
            try:
                logger.info("Using Node.js systeminformation for hardware info")
                node_info = get_hardware_info_nodejs()
                if node_info:
                    logger.info("Successfully got hardware info from Node.js")
                    return node_info
            except Exception as e:
                logger.warning(
                    f"Node.js hardware info failed: {e}, falling back to Python"
                )

        # Fallback to Python methods
        logger.info("Using Python WMI/psutil for hardware info")
        info = {}
...
class HardwareBenchmarkAgent:
    ...
    def __init__(self, server_url: str, api_key: Optional[str] = None):
        ...
        if GitHubUpdater:
            self.updater = GitHubUpdater(
                self.GITHUB_OWNER, self.GITHUB_REPO, self.CURRENT_VERSION
            )
    ...
    def start_execution(
        self, script_id: Optional[str], task_id: Optional[str]
    ) -> Optional[str]:
    ...
    def _report_software_error(self, task_id: str, error_message: str):
        ...
        try:
            ...
            requests.put(
                url,
                json={"task_status": "failed", "error_message": error_message},
                timeout=10,
            )
        except Exception as e:
    ...
    def execute_task(self, task: Dict[str, Any]):
        ...
        logger.info(
            f"Executing task: {task_name} (task_id: {task_id}, type: {task_type})"
        )
        ...
        if software_list and self.software_manager:
            ...
            for software in software_list:
                software_name = software.get(
                    "software_name", software.get("software_code", "unknown")
                )
                ...
                logger.info(
                    f"Software {software_name} not found, preparing to install..."
                )
        ...
        if software:
            ...
            script_config["process"] = (
                software.get("name", "").split()[0] if software.get("name") else ""
            )
        ...
        try:
            ...
            if self.metrics_collector:
                collection_thread = threading.Thread(
                    target=self._collect_metrics_loop, args=(60, metrics_history)
                )
            ...
            self.complete_execution(
                execution_id=execution_id, exit_code=-1, error_message=str(e)
            )
    ...
    def send_metrics(self) -> bool:
        ...
        try:
            ...
            data = {
                ...
                "memory_used_mb": sys_metrics.get("ram_used_gb", 0)
