...
调用 Node.js systeminformation 库获取准确的硬件信息
"""
...
# Node.js 脚本路径
NODE_SCRIPT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "nodejs", "hardware_info"
)
...
def get_hardware_info() -> Dict[str, Any]:
    ...
    if not is_node_available():
        logger.warning("Node.js not available, using fallback")
        return {}
    ...
    if result and result.get("success"):
        ...
        info["cpu_threads"] = cpu.get(
            "cores", 0
        )  # systeminformation 使用 cores 作为逻辑处理器
...
if __name__ == "__main__":
    ...
    if is_node_available():
        ...
        print("Please install Node.js to use systeminformation")
