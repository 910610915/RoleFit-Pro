...
```sql
-- 创建数据库
CREATE DATABASE hardware_benchmark;

-- 创建用户
CREATE USER benchmark_user WITH PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE hardware_benchmark TO benchmark_user;
```
...
# 2. 安装必要软件
apt update && apt install -y python3 python3-pip nodejs npm postgresql redis-server nginx

# 3. 配置数据库
sudo -u postgres createuser benchmark_user
sudo -u postgres createdb hardware_benchmark
sudo -u postgres grant all on database hardware_benchmark to benchmark_user
