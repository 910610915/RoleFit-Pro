...
class TaskScheduler:
        ...
        try:
            ...
            for task in tasks:
                next_run = self.calculate_next_run(
                    task.cron_expression, task.completed_at
                )
    ...
    def calculate_next_run(
        self, cron_expression: Optional[str], last_run: datetime
    ) -> Optional[datetime]:
        """根据cron表达式计算下次执行时间"""
    ...
    async def cleanup_old_data(self):
        ...
        try:
            ...
            result = retention_service.run_cleanup(dry_run=False)
            logger.info(
                f"Data cleanup completed. Deleted {result.get('total_deleted', 0)} records"
            )
