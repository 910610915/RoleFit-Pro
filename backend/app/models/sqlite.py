...
class JobScript(Base):
    ...
    script_name = Column(String(200), nullable=False)
    script_code = Column(String(50), unique=True, nullable=False)
    position_id = Column(String(36), ForeignKey("positions.id"), nullable=True)  # 兼容旧字段
...
class AlarmRule(Base):
    ...
    alarm_type = Column(String(50), nullable=False)  # device_offline, test_failed, score_low, etc.
    condition = Column(Text, nullable=False)  # JSON condition
    threshold = Column(Float, nullable=True)
    enabled = Column(Boolean, default=True)
    notification_channels = Column(Text, nullable=True)  # JSON array: email, sms, webhook
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
