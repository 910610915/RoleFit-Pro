...
async def list_alarm_rules(
    ...
    return [
        AlarmRuleResponse(
            ...
            enabled=r.enabled,
            notification_channels=json.loads(r.notification_channels) if r.notification_channels else [],
...
async def create_alarm_rule(
    ...
    return AlarmRuleResponse(
        ...
        notification_channels=json.loads(db_rule.notification_channels) if db_rule.notification_channels else [],
...
async def check_alarms(
    ...
    return {"checked": checked_count, "message": f"检查完成，发现 {checked_count} 个新告警"}
