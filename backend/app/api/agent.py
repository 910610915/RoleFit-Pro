...
from app.models.sqlite import JobScript, ScriptExecution, TestTask, TestSoftware, Device, SoftwareMetrics
...
async def get_pending_tasks(
    ...
    for task in tasks:
        ...
        target_devices = []
        try:
            target_devices = json.loads(task.target_device_ids) if task.target_device_ids else []
        except:
            pass
    ...
    if metrics_data:
        for m in metrics_data:
            metric = SoftwareMetrics(
                ...
                timestamp=datetime.fromisoformat(m.get("timestamp", datetime.utcnow().isoformat())),
...
async def get_script_detail(
    ...
    """获取脚本详情"""
    result = await db.execute(
        select(JobScript).where(JobScript.id == script_id)
    )
