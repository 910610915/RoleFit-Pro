...
def task_to_response(task: TestTask) -> dict:
    ...
    for column in task.__table__.columns:
        ...
        if (
            column.name
            in ["target_device_ids", "target_departments", "target_positions"]
            and value
        ):
            try:
...
async def create_task(
    ...
    if task_data.test_script_id:
        ...
        if not script:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Test script not found"
            )
...
async def list_tasks(
    ...
    query = (
        query.order_by(TestTask.created_at.desc())
