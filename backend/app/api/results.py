...
from app.models.sqlite import TestResult, Device, TestTask, PositionStandard, ScriptExecution, SoftwareMetrics
from app.schemas.result import (
    TestResultCreate, TestResultUpdate, TestResultResponse,
    TestResultListResponse, ResultStatistics, DevicePerformanceSummary
)
...
async def create_result(
    ...
    result = await db.execute(
        select(Device).where(Device.id == result_data.device_id)
    )
...
async def list_results(
    ...
    query = query.order_by(TestResult.start_time.desc()).offset((page - 1) * page_size).limit(page_size)
...
async def compare_devices(
    ...
    for device_id in device_id_list:
        ...
        # 获取该设备的最新测试结果
        result_query = select(TestResult).where(
            TestResult.device_id == device_id
        ).order_by(TestResult.start_time.desc()).limit(1)
        ...
        if latest_result and latest_result.cpu_score and latest_result.gpu_score and latest_result.memory_score and latest_result.disk_score:
            calculated_overall = (latest_result.cpu_score + latest_result.gpu_score + latest_result.memory_score + latest_result.disk_score) / 4
        ...
        comparison.append({
            ...
            "latest_result": {
                ...
                "memory_score": latest_result.memory_score if latest_result else None,
                ...
                "test_time": latest_result.start_time.isoformat() if latest_result and latest_result.start_time else None
...
async def get_result_metrics(
    ...
    if not execution:
        # 尝试通过其他方式查找
        exec_result = await db.execute(
            select(ScriptExecution).where(ScriptExecution.task_id.like(f"%{result_id}%"))
        )
        execution = exec_result.scalar_one_or_none()
